...
class Controller {
    ...
    startBootApps(debug) {
        return __awaiter(this, void 0, void 0, function* () {
            const appsToStart = yield vscode.window.showQuickPick(this.getAppList().filter(app => app.state !== BootApp_1.AppState.RUNNING).map(app => ({ label: app.name, path: app.path })), /** items */ { canPickMany: true, placeHolder: `Select apps to ${debug ? "debug" : "start"}.` } /** options */);
            if (appsToStart !== undefined) {
                const appPaths = appsToStart.map(elem => elem.path);
                yield Promise.all(this.getAppList().filter(app => appPaths.indexOf(app.path) > -1).map(app => this.startBootApp(app)));
            }
        });
    }
    startBootApp(app, debug) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield vscode.debug.startDebugging(vscode.workspace.getWorkspaceFolder(vscode.Uri.parse(app.path)), Object.assign({}, targetConfig, {
                noDebug: !debug,
                vmArgs
            }));
        });
